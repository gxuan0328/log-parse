...
def test_anchor_counts_match_template() -> None:
    rows = aggregate.build(_anchor_full_state())
    assert [row.count for row in rows] == [1, 1, 1, 1, 1, 1, 3, 1, 7, 1, 1]


def test_anchor_counts_sum_to_19() -> None:
    rows = aggregate.build(_anchor_full_state())
    assert sum(row.count for row in rows) == 19
...
def test_anchor_orphan_excluded_ip_count_is_1() -> None:
    ...
    # row 8, same IP) never reaches state -- proven by COUNT staying at
    # 1, not 2.
    rows = aggregate.build(_anchor_full_state())
...
def test_anchor_row_shape_matches_report_row() -> None:
    ...
    assert rows[0] == ReportRow(
        client_ip="10.243.129.44", hosp_id="1145010038", hosp_abbr="門諾醫院", count=1
    )
