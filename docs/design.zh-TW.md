...
**`conf/test_hosts.conf`** — 每行一個 IPv4 或 CIDR 網段，可加 `#` 註解。
shipped 配置為僅限外部使用者的生產政策：`192.168.0.0/16`（涵蓋所有內部
192.168.x.x 來源）加上明確主機 `10.252.130.178`。此政策下 `192.168.139.28`
（`/health` 探針，已由過濾器 1 排除）、`192.168.139.110`（QA 主機，每週 209 筆）
與 `192.168.139.119`（內部生產閘道，sample 中每週約 712 筆 IIS）皆屬內部、
非外部流量而被排除——閘道刻意納入，因私有網段閘道屬內部基礎設施，非外部使用者。
回歸測試套件與隨附 sample demo 則改用 `tests/fixtures/test_hosts.conf`（一份不列
`.119` 的明確 IP QA 集，使閘道維持業務），讓 sample 的 IIS／overview 報告保有可測
資料；shipped 的 `/16` 政策另由測試 J16／J24–J27／J29 驗證。
